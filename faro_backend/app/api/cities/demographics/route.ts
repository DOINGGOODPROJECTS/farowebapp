...
const DATA_PATH = path.join(process.cwd(), 'data', 'Demographics.csv');
...
const loadDemographicsRows = async (): Promise<DemographicsRow[]> => {
  ...
  for (const line of dataLines) {
    ...
    const [cityId, cityRaw, blackPopulationRaw, housingRaw] = cells;
    const [namePart, statePart] = cityRaw.split(',').map((part) => part.trim());
    rows.push({
      cityId,
      name: namePart,
      stateCode: statePart || '',
      blackPopulationPct: parseNumber(blackPopulationRaw),
      housingIndex: parseNumber(housingRaw),
    });
  }
  return rows;
};
...
export async function GET(req: NextRequest) {
  ...
  try {
    const rows = await loadDemographicsRows();
    ...
    return NextResponse.json({
      blackPopulationPct: match.blackPopulationPct,
