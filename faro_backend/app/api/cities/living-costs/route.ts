...
const DATA_PATH = path.join(process.cwd(), 'data', 'LivingCosts.csv');
...
const loadLivingCostRows = async (): Promise<LivingCostRow[]> => {
  ...
  const rows: LivingCostRow[] = [];
  for (const line of dataLines) {
    const cells = parseCsvLine(line);
    if (cells.length < 4) continue;
    const [cityId, cityRaw, costRaw, housingRaw] = cells;
    const [namePart, statePart] = cityRaw.split(',').map((part) => part.trim());
    rows.push({
      cityId,
      name: namePart,
      stateCode: statePart || '',
      costIndex: parseNumber(costRaw),
...
export async function GET(req: NextRequest) {
  ...
  try {
    const rows = await loadLivingCostRows();
