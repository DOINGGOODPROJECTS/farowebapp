...
const loadCityRows = async (): Promise<CityRow[]> => {
  ...
  for (const line of dataLines) {
    ...
    const [cityId, cityRaw, populationRaw, medianIncomeRaw] = cells;
    const [namePart, statePart] = cityRaw.split(',').map((part) => part.trim());
    rows.push({
      cityId,
      name: namePart,
      stateCode: statePart || '',
      population: parsePopulation(populationRaw),
      populationRaw,
      medianIncomeRaw,
    });
  }
