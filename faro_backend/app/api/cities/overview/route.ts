...
const DATA_PATH = path.join(
  process.cwd(),
  'data',
  'CitiesIncomePopulationMedianScore.csv',
);
...
const loadOverviewRows = async (): Promise<OverviewRow[]> => {
  ...
  for (const line of dataLines) {
    ...
    const [cityId, cityRaw, populationRaw, medianIncomeRaw, notes] = cells;
    const [namePart, statePart] = cityRaw.split(',').map((part) => part.trim());
    rows.push({
      cityId,
      name: namePart,
      stateCode: statePart || '',
      population: parseNumber(populationRaw),
      populationRaw,
      medianIncome: parseNumber(medianIncomeRaw),
      medianIncomeRaw,
      notes: notes || '',
    });
  }
